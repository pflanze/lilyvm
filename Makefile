...
target-local/vmtest: .deps target-local $(OBJS) target-local/run_tests.o target-local/vmtest.o
	$(LINK) -o target-local/vmtest $(OBJS) target-local/run_tests.o target-local/vmtest.o

target-local/lilyvm: .deps target-local $(OBJS) target-local/lilyvm.o
	$(LINK) -o target-local/lilyvm $(OBJS) target-local/lilyvm.o
...
target-local/%.E: %.c
	$(COMPILER) -E -DFIL=`../chj-ctest/bin/path-to-FIL $<` -o $@ $<
...
target-local/%.s: target-local/%.c
	$(COMPILER) -S -DFIL=`../chj-ctest/bin/path-to-FIL $<` -o $@ $<
...
target-local/%.o: ../chj-ctest/%.c
	$(COMPILER) -c -DFIL=`../chj-ctest/bin/path-to-FIL $<`  -o $@ $<
else
