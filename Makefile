...
CC=$(COMPILER) $(CFLAGS) $(SAN)
AS=$(ASSEMBLER) -c
LD=$(COMPILER) $(LDFLAGS) $(SAN)
...
target-local/vmtest: .deps target-local $(OBJS) target-local/run_tests.o target-local/vmtest.o
	$(LD) -o target-local/vmtest $(OBJS) target-local/run_tests.o target-local/vmtest.o

target-local/lilyvm: .deps target-local $(OBJS) target-local/lilyvm.o
	$(LD) -o target-local/lilyvm $(OBJS) target-local/lilyvm.o
...
target-local/%.E: %.c
	$(CC) -E -DFIL=`../chj-ctest/bin/path-to-FIL $<` -o $@ $<
...
target-local/%.s: target-local/%.c
	$(CC) -S -DFIL=`../chj-ctest/bin/path-to-FIL $<` -o $@ $<
...
target-local/%.o: ../chj-ctest/%.c
	$(CC) -c -DFIL=`../chj-ctest/bin/path-to-FIL $<`  -o $@ $<
else
